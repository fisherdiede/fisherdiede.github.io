...
var FADE_START_TIME = 7; // When to start fading out
...
function draw() {
	...
	if (!showProfile) {
		// Welcome screen with fade out animation
		let alpha = 255 * (1 - fadeAmount);
		...
		normalizedDist = constrain(normalizedDist, 0, 1);

		// Apply exponential curve (inverse so closer = brighter)
		// Using power of 3 for a strong exponential effect
		...
		brightness = brightness * (1 - fadeAmount);

		// Draw black background for button with fade
		fill(0, brightness);
		noStroke();
		let padding = 20;
		let rectW = 200;
		...
		// Profile screen with fade in animation
		...
		// Scale to cover the circle (crop edges if needed)
		if (imgAspect > 1) {
			// Wider than tall
			imgH = circleDiameter;
